...
def get_all_files(directory, month, level): #디렉토리와 하위폴더 내 전체 파일의 경로+이름을 반환하는 함수
    ...
    # 디렉토리 내의 파일과 폴더 목록을 가져옴
        ...
        for item in items:
          ...
          if os.path.isfile(item_path) and item_path.find(str(month)+'월')!=-1 and item_path.find(str(level)+'차')!=-1: #조건에 맞는 파일 반환
            # 파일인 경우 파일 목록에 추가
...
folder_path = './'
create_folder(folder_path+"[심화R&D프로젝트] "+str(month)+"월 "+str(level)+"차 bi-weekly 보고서 리스트")
files_path = get_all_files(folder_path, month, level)
for input_file_path in files_path:
    if os.path.isfile(input_file_path) and os.path.splitext(input_file_path)[1] in ['.hwp','.doc','.docx', '.txt'] :  # 폴더가 아닌 파일만 처리
        convert_word_to_pdf(input_file_path, "./[심화R&D프로젝트] "+str(month)+"월 "+str(level)+"차 bi-weekly 보고서 리스트/"+os.path.splitext(os.path.basename(input_file_path))[0])